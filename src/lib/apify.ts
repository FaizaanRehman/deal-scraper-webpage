...
export async function fetchInstagramPosts(): Promise<InstagramPost[]> {
  ...
  if (!actorId || !token) {
    throw new Error(
      'Missing APIFY_ACTOR_ID or APIFY_API_TOKEN environment variables.'
    );
  }
  ...
  if (!datasetId) {
    console.error(
      `[Apify] No dataset ID returned from actor run ${actorRun.id}`
    );
  ...
  while (true) {
    const { items, total } = await client
      .dataset(datasetId)
      .listItems({ limit: batchSize, offset });
    posts.push(
      ...items.map((item) => ({
